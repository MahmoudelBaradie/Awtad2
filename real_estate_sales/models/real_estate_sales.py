...
class RealEstateSale(models.Model):
    ...
    _inherit = ['real.estate.accounting.mixin', 'mail.thread']
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('real.estate.sale') or 'New'
    ...
    def action_post_sale(self):
        ...
        for rec in self:
            if rec.state != 'contracted':
                raise UserError(_('Only contracted sales can be posted.'))
